using System;
using System.Threading.Tasks;
using Xunit;

namespace SKIT.FlurlHttpClient.Upyun.Uss.UnitTests
{
    public class TestCase_ApiExecuteFormTests
    {
        [Fact(DisplayName = "测试用例：调用 API [POST] /{bucket}")]
        public async Task TestExecuteUploadFileWithForm()
        {
            var request = new Models.UploadFileWithFormRequest()
            {
                BucketName = TestConfigs.UpyunBucketName,
                FileKey = "/test/sample.jpg",
                FileBytes = Convert.FromBase64String("")
            };
            var response = await TestClients.Instance.ExecuteUploadFileWithFormAsync(request);

            Assert.True(response.IsSuccessful());
        }
    }
}
